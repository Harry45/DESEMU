...
## NUTS
echo Sampling with NUTS with stepsize of 0.01 and tree depth of 8 nwarmup is 500 and nsamples is 15000
python sample.py --config=config.py:desyr1 --config.sampler=nuts --config.nuts.nwarmup=500 --config.nuts.nsamples=15000 --config.nuts.nchain=2 --config.nuts.chainmethod=vectorized --config.use_emu=True --config.samplername=small_ss_high_td_glamdring_experimental
...
## Submitting Job
# addqueue -s -q gpulong -m 10 --gpus 1 ./runme.sh